...
while (repeticao>numero):
    ...
    vetor_inter_gateways = np.asarray(teste)
    ...
    mapa.save('arquivos de saida/'+str(numero)+'aeroporto.html')
    ...
    for n in range(0,19598):
        ...
        if temp2 == 3:
            ...
            linhas = demanda.loc[demanda['ID_AEROPORTO_1']==float(local1)]
            linha = linhas.loc[linhas['ID_AEROPORTO_2']==float(local2)]
            temp5 = linha['DEMANDA'].astype(float) * 1
            temp6 = float(temp5)
